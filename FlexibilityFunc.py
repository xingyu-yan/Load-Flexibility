...
def FlexibilityCalculate(Load, Load_Name):
    ...
    x = Load[0:(24*NumDayPlot)]
    y = np.array(Load[0:(24*NumDayPlot)]) - np.array(Load_Mat_Dif_toList_Days[0])
    plt.figure()
    ...
    x = Load_Mat_Week_Day_toList_Weeks[0]
    y = Load_Mat_Week_Day_toList_Weeks[0] - np.array(Load_Mat_Week_Day_Dif_toList_Weeks[0])
    plt.figure()
